...
export const CATEGORY_HOOK: Record<string, { 짚기: string; 제안: string }> = {
  '3D프린터 도입 가이드|도입검토': {
    짚기:
      '사양표의 최대 조형 크기와 해상도만 비교하시기 쉬워요. 그런데 실제 도입에서 갈리는 것은 **우리 부품이 그 장비에서 실제로 나오는가**예요. 같은 사양이라도 소재 제약과 후처리 공정이 달라 결과물이 다릅니다.',
    제안:
      '그래서 저희는 도입 전에 실제 부품으로 샘플 출력을 해 보시라고 권해요. 도면을 주시면 맞는 방식과 소재를 골라 출력 조건까지 정리해 드립니다.',
  },
  '소재 가이드|도입검토': {
    짚기:
      '데이터시트의 인장강도 숫자만 보고 고르시기 쉬워요. 그런데 실제로 갈리는 것은 **출력 방향과 후처리에 따른 물성 변화**예요. 같은 소재라도 조형 방향이 다르면 강도가 다르게 나옵니다.',
    제안:
      '그래서 저희는 부품의 하중 방향과 사용 환경을 먼저 여쭤봐요. 조건에 맞는 소재 후보를 좁히고, 필요하면 시편 출력으로 검증한 뒤 정합니다.',
  },
}
