...
export const CATEGORY_INTRO: Record<string, string> = {
  '출력 의뢰·비용':
    '견적에 필요한 파일과 조건, 예상 비용과 납기, 발주 전에 확인할 항목을 정리했습니다.',
  '공정·소재 선택':
    '부품의 강도와 표면, 사용 환경에 맞춰 출력 방식과 소재 후보를 좁힐 수 있습니다.',
  '설계·품질 문제':
    '공차와 강도, 변형, 조립 문제의 원인을 확인하고 설계에서 고칠 항목을 찾을 수 있습니다.',
  '후처리·장비 운영':
    '표면 마감과 도색, 세척, 서포트 제거, 장비 유지관리 방법을 확인할 수 있습니다.',
  '3D프린터 도입':
    '외주와 장비 구매를 비교하고, 회사의 부품과 운영 조건에 맞는 장비를 검토할 수 있습니다.',
  '산업별 제작 사례':
    '같은 업종과 비슷한 부품에서 3D프린팅을 어떤 방식으로 적용했는지 확인할 수 있습니다.',
}
...
/** 세부분류는 글 수가 아니라 독자가 판단하는 순서로 고정한다. */
export const SUBCATEGORY_ORDER: Record<string, string[]> = {
  '출력 의뢰·비용': ['의뢰 준비', '견적·비용', '납기·수량', '업체 선정·발주', '검수·재주문'],
  '공정·소재 선택': [
    '공정 비교',
    '소재 선택·범용 플라스틱',
    '고성능·기능성 소재',
...
export function subcategoryRank(category: string, subcategory: string): number {
  const index = (SUBCATEGORY_ORDER[category] ?? []).indexOf(subcategory)
...
export const CATEGORY_HOOK: Record<string, { 짚기: string; 제안: string }> = {
  '3D프린터 도입|도입검토': {
    짚기:
      '사양표의 최대 조형 크기와 해상도만 비교하시기 쉽습니다. 실제 도입에서는 **우리 부품을 그 장비로 만들 수 있는가**를 먼저 확인해야 합니다. 같은 사양이라도 소재 제약과 후처리 공정이 달라 결과물이 다릅니다.',
    제안:
      '그래서 저희는 도입 전에 실제 부품으로 샘플 출력을 해 보시라고 권합니다. 도면을 주시면 맞는 방식과 소재를 골라 출력 조건까지 정리해 드립니다.',
  },
  '공정·소재 선택|도입검토': {
    짚기:
      '데이터시트의 인장강도 숫자만 보고 고르시기 쉽습니다. **출력 방향과 후처리에 따른 물성 변화**도 함께 확인해야 합니다. 같은 소재라도 조형 방향이 다르면 강도가 다르게 나옵니다.',
    제안:
